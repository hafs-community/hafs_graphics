...
STORM=${STORM:-NATL}
STORMID=${STORMID:-00L}
YMDH=${YMDH:-2020070506}
HOMEgraph=/work/noaa/hwrf/save/${USER}/HAFSGraphics/hafs_graphics #/mnt/lfs4/HFIP/hwrfv3/${USER}/hafs_graphics
...
COMhafs=/work/noaa/hwrf/save/${USER}/HAFSGraphics/EMC_HAFSJ_2020rt/com #/your/hafs/com/dir

export HOMEgraph=${HOMEgraph:-/work/noaa/hwrf/save/${USER}/HAFSGraphics/hafs_graphics}
export USHgraph=${USHgraph:-${HOMEgraph}/ush}
export DRIVERDOMAIN=${USHgraph}/driverDomain.sh

export COMhafs=${COMhafs:-${COMhafs}/${YMDH}/${STORMID}}
...
do
  echo "${APRUNS} ${DRIVERDOMAIN} $stormModel $STORM $STORMID $YMDH False $is6Hr ${figScriptAll[$i]} ${figNameAll[$i]} ${standardLayerAll[$i]} $figTimeLevel $figTimeLevel > ${WORKgraph}/$STORM$STORMID.$YMDH.${figNameAll[$i]}.$figTimeLevel.log 2>&1 ${BACKGROUND}" >> $cmdfile
done
...
for figTimeLevel in ${figTimeLevels};
do
  echo "${APRUNS} ${DRIVERDOMAIN} $stormModel $STORM $STORMID $YMDH True $is6Hr ${figScriptAll[$i]} ${figNameAll[$i]} ${standardLayerAll[$i]} $figTimeLevel $figTimeLevel > ${WORKgraph}/$STORM$STORMID.$YMDH.${figNameAll[$i]}.$figTimeLevel.log 2>&1 ${BACKGROUND}" >> $cmdfile
done
