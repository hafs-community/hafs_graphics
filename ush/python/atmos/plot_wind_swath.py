...
for i,r in enumerate(freq):
    ...
    print(np.nanmin(wind_mask))
    print(np.nanmax(wind_mask))
...
try:
    #cf = ax.contourf(lon, lat, wind, cflevels, cmap=cm, norm=norm, transform=transform,extend='both')
    cf = ax.contourf(lonn[:,sort_lon], lat, wind_masked, cflevels, cmap=cm, norm=norm, transform=transform,extend='both')
    cbshrink = 1.0
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=50, shrink=cbshrink, extendrect=True, 
                  ticks=[30,34,50,64,83,96,114,135])
    cb.ax.set_yticklabels(['30','34','50','64','83','96','114','135'])
except:
    print('ax.contourf failed, continue anyway')

try:
    #ax.contour(lon, lat, wind,['30','34','50','64','83','96','114','135'],colors='grey',linewidths=0.7)
    ax.contour(lonn[:,sort_lon], lat, wind_masked,['30','34','50','64','83','96','114','135'],colors='grey',linewidths=0.7,transform=transform)
except:
...
title_center = 'Wind Swath (knots)'
ax.set_title(title_center, loc='center', y=1.05)
title_left = conf['stormModel']+' '+conf['stormName']+conf['stormID']
ax.set_title(title_left, loc='left')

title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+' for ' + str(accumulation_time) + ' h'
ax.set_title(title_right, loc='right',x=1.05)

#plt.show() 
fig_name = fig_prefix+'.'+'wind_swath.'.lower()+conf['fhhh']+'.png'
