...
fig_name = fig_prefix+'.storm.'+'crs_ew_rh_tempanomaly_wind_rvor.'+conf['fhhh'].lower()+'.png'
cbshrink = 1.0
...
if np.max(tmp_anomaly[:,100:700, idx[1]]) > 4:
  ...
  cf2 = ax2.contour(newlons, levs, ( tmp_anomaly[:, idx[0], 200:800] ), cflevels, colors='red', linestyles='dotted')
  plt.clabel(cf2, cflevels, fontsize=10)

if np.max(vorttmp[:, 100:700 , idx[1]]*10**5) > 30:
  cflevels = [50,100,150]
  cf3 = ax2.contour(newlons, levs, ( vorttmp[:, idx[0], 200:800]*10**5 ), cflevels, colors='black')
