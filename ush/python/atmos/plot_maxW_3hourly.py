...
varlen=grbswath.select(shortName='DZDT', statisticalProcess = 'Maximum')
print('varlen',len(varlen))
varshape=grbswath.select(shortName='DZDT', statisticalProcess = 'Maximum')[0].data()
print('varshape',varshape.shape)

#wind_maskedtmp = np.zeros((varshape.shape[0],varshape.shape[1]))

for ind, frame in enumerate(range(0,len(varlen)+1,2)):
  print(ind,frame)
  ...
  try:
    ...
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=50, shrink=cbshrink, extendrect=True, 
                    ticks=[1,2,3,5,7,9,11,15,20,30])
    cb.ax.set_yticklabels(['1','2','3','5','7','9','11','15','20','30'])
  except:
  ...
  title_center = '3 Hour Max. W below 400 hPa (m/s)'
  ...
  #plt.show() 
  fig_name = fig_prefix+'.maxW'+'.f'+conf['fhhh']+'.png'
