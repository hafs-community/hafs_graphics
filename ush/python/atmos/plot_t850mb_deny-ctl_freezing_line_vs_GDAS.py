...
"""This script is to plot out GFS 850 mbar temp differences and freezing line vs GDAS."""
...
print('Extracting Temperature 850 mbar')
levstr='850 mb'
...
fig_name = fig_prefix+'.'+'t850m_freezing_line_GDAS.'+conf['ymdh']+'.'+conf['fhhh'].lower()+'.png'
cbshrink = 0.7
lonmin = conf['lonmin']
lonmax = conf['lonmax']
lonint = 10.0
...
ax.contour(lon, lat, tmp_ctl, levels=[0], colors='mediumaquamarine', linewidths=2)
handle1 = plt.plot([0], [0], color='mediumaquamarine', lw=2, label=conf['model_ctl'])
ax.contour(lon, lat, tmp_deny, levels=[0], colors='grey', linewidths=2)
handle2 = plt.plot([0], [0],color='grey', lw=2, label=conf['model_deny'])
ax.contour(lon, lat, tmp_gdas, levels=[0], colors='black', linewidths=2)
handle3 = plt.plot([0], [0], color='black', lw=2, label='GDAS')
