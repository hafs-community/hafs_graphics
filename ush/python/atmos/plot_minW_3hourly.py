...
varlen=grbswath.select(shortName='DZDT', statisticalProcess = 'Minimum')
print('varlen',len(varlen))
varshape=grbswath.select(shortName='DZDT', statisticalProcess = 'Minimum')[0].data()
print('varshape',varshape.shape)

for ind, frame in enumerate(range(1,len(varlen)+1,2)):
  print(ind,frame)
  conf['fhhh'] = "{:03d}".format((ind+1)*3)
  conf['fcstTime'] = pd.to_timedelta(int(conf['fhhh']), unit='h')
  conf['validTime'] = conf['initTime'] + conf['fcstTime']

  if frame > 1:
    wind = grbswath.select(shortName='DZDT',statisticalProcess = 'Minimum')[frame].data()
    wind.data[wind.mask] = np.nan
    wind0= grbswath.select(shortName='DZDT',statisticalProcess = 'Minimum')[frame-2].data()
    wind0.data[wind0.mask] = np.nan
    wind=wind-wind0
  elif frame == 1:
    wind = grbswath.select(shortName='DZDT',statisticalProcess = 'Minimum')[frame].data()
    wind.data[wind.mask] = np.nan
  ...
  for i,r in enumerate(freq+1):
      ...
      wind_mask = wind*okR_matrix
      wind_mask[wind_mask == 0] = np.nan
      wind_masked0[i,:,:] = wind_mask
  
  wind_masked = np.nanmean(wind_masked0,axis=0)

  #===================================================================================================
  print('Plotting DZDTmin')
  ...
  cflevels = [-10,-7,-5,-4,-3,-2.5,-2,-1.5,-1,-0.5]               # white

  cfcolors = [ 'magenta','maroon','red','orange','yellow','green','lawngreen','cyan', 'white',]
  
  cm = matplotlib.colors.ListedColormap(cfcolors)
  norm = matplotlib.colors.BoundaryNorm(cflevels, cm.N)  
  
  try:
    cf = ax.contourf(lon, lat, wind_masked, cflevels, cmap=cm, norm=norm, transform=transform,extend='both')
    cbshrink = 1.0
    cb = plt.colorbar(cf, orientation='vertical', pad=0.02, aspect=50, shrink=cbshrink, extendrect=True, 
                    ticks=[-10,-7,-5,-4,-3,-2.5,-2,-1.5,-1,-0.5])
    cb.ax.set_yticklabels(['-10','-7','-5','-4','-3','-2.5','-2','-1.5','-1','-0.5'])
  except:
    print('ax.contourf failed, continue anyway')
  
  ax.plot(lon_adeck[0:ind+2], lat_adeck[0:ind+2], '.-k', markersize=1, linewidth=0.5)
  ...
  title_center = '3 Hour Min. W below 400 hPa (m/s)'
  ...
  #plt.show() 
  fig_name = fig_prefix+'.minW'+'.f'+conf['fhhh']+'.png'
  plt.savefig(fig_name, bbox_inches='tight')
  plt.close(fig)

  if conf['fhhh'] == fhour[len(fhour)-1] :
    print('Stop at',fhour[len(fhour)-1],conf['fhhh'],ind)
    sys.exit()
