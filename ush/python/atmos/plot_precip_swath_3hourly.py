...
for ind, frame in enumerate(range(0,len(varlen),2)):
  ...
  try:
      cf = ax.contourf(lon, lat, apcp_masked*3600*3*(ind+1), cflevels, cmap=cm, norm=norm, transform=transform) #accumulated to fhhh from prate (apcp_masked*3600*frame)
      cbshrink = 1.0
  ...
  title_center = 'Rain Swath (in)'
  ax.set_title(title_center, loc='center', y=1.05)
  title_left = conf['stormModel']+' '+conf['stormName']+conf['stormID']
  ax.set_title(title_left, loc='left')
  
  title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+'F'+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
  ax.set_title(title_right, loc='right',x=1.05)
  
  #plt.show() 
  fig_name = fig_prefix+'.precip.swath.'+'f'+conf['fhhh']+'.png'
