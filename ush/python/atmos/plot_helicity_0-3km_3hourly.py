...
varlen=grbswath.select(shortName='UPHL', level='3000-0 m above ground')
print(len(varlen))
varshape=grbswath.select(shortName='UPHL', level='3000-0 m above ground')[0].data()
...
for frame in range(len(varlen)):
  ...
  uphl = grbswath.select(shortName='UPHL', level='3000-0 m above ground')[frame].data()
  uphl.data[uphl.mask] = np.nan
  #===================================================================================================
  ...
  title_center = '3 Hour Max. 0-3km Updraft Helicity (m^2/s^2)'
  ...
  #plt.show() 
  fig_name = fig_prefix+'.helicity.0-3km'+'.f'+conf['fhhh']+'.png'
