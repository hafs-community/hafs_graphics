...
if conf['stormDomain'] == 'grid02':
    ...
    fig_name = fig_prefix+'.storm.'+'lhtflux_wind10m.'+conf['fhhh'].lower()+'.png'
    cbshrink = 1.0
    ...
    fig_name = fig_prefix+'.'+'lhtflux_wind10m.'+conf['fhhh'].lower()+'.png'
    cbshrink = 1.0
    lonmin = np.min(lon)
    lonmax = np.max(lon)
    lonint = 10.0
...
cflevels = np.linspace(-200, 1800, 41)
ctmp = plt.get_cmap('turbo')
