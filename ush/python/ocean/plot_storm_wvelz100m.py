...
"""This scrip plots the vertical velocity at 100 meters for an area 500 km around the storm eye. """
...
if ocean == 'mom6':
    ...
    print('The calculation of vertical velocity for MOM6 has not yet been implemented')
    sys.exit()

if ocean == 'hycom':
    varr000 = np.asarray(nc000['w_velocity'][0,:,:,:])
...
#================================================================
# Calculate wvelz100
ok100 = np.where(zl <= 100)[0][-1]

varr000 = varr000[ok100,:,:]
varr = varr[ok100,:,:]
...
if len(lon_adeck[okfhour])!=0 and len(lat_adeck[okfhour])!=0:
    if lat_adeck[nhour] < (latmax+5.0):
        ...
        var_info = '100 m Vertical Velocity (m/s)'
        ...
        model_info = os.environ.get('TITLEgraph','').strip()
        var_info = '100 m Vertical Velocity Change (m/s)'
        storm_info = conf['stormName']+conf['stormID']
        title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
        ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
        title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
