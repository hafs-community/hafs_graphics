...
"""This scrip plots the sea surface salinity for the entire ocean domain. """ 
...
if ocean == 'mom6':
    var = np.asarray(nc['SSS'][0,:,:])
    lon = np.asarray(nc.xh)
    lat = np.asarray(nc.yh)

if ocean == 'hycom':
    var = np.asarray(nc['salinity'][0,0,:,:])
...
lb = ax.contour(lon, lat, var, levels=[36], colors='grey', alpha=0.7,transform=ccrs.PlateCarree(),linewidths=0.5)
...
var_info = 'Sea Surface Salinity (psu)'
