...
def xy4NDBC(IDofI):   
   ...
   intxt='/gpfs/dell2/emc/modeling/noscrub/hsk/Data/NDBC//NDBCstations.txt'
...
def find_dist(x,y,xc,yc):
    """ return a distance in km"""
    dx = x - xc
    dy = y - yc
    out = 60 * 1.8533 * np.sqrt( dy**2 + (dx + np.cos(np.radians(y)))**2)
    return out
