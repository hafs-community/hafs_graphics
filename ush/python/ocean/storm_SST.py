 ...
 ****************************************************************************
 usage: python storm_SST.py stormModel stormName stormID YMDH trackon COMhafs
 -----
 ****************************************************************************
...
skip=3
u0=ncfiles['u_velocity'].isel(Z=0)[:,::skip,::skip]
v0=ncfiles['v_velocity'].isel(Z=0)[:,::skip,::skip]
ln=lns[::skip,::skip]
lt=lts[::skip,::skip]
del ncfiles
dummy=np.ones(lns.shape)
#stats=[]		# (mean,min,max) for SST 
			# and (mean,min.max) for SSTcooling
...
for k in range(len(aln)):
     #print(' --- storm_SST.py ---')
     #print(' ... # file=',k,' out of',var.shape[0])
     #print()
     ...
     plt.suptitle(storm.upper()+' '+tcid.upper()+'  '+'Ver Hr '+"%3d"%(fhr)+'  (IC='+cycle+'): SST & Change [$^o$C]',fontsize=15)
     plt.subplot(121)
     (var[k]*dumb).plot.contourf(levels=np.arange(26,31.2,0.2),extend='both',cmap='RdBu_r')
     ...
     pngFile=os.path.join(graphdir,aprefix.upper()+'.'+model.upper()+'.storm.SST.currents.f'+"%03d"%(fhr)+'.png')
