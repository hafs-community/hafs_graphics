...
if ocean == 'hycom':
    fname =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.hycom.2d.'+conf['fhhh']+'.nc'
...
if ocean == 'mom6':
    var = np.asarray(nc['SSH'][0,:,:])
    lon = np.asarray(nc.xh)
    lat = np.asarray(nc.yh)

if ocean == 'hycom':
    var = np.asarray(nc['sea_surface_height'][0,:,:])/100 #convert cm to m
