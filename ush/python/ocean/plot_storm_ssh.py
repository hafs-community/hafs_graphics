...
"""This scrip plots the sea surface height for an area 500 km around the storm eye. """ 
...
if ocean == 'mom6':
    ...
    if os.path.exists(ncfile):
        ...
        print('file ',ncfile,' does not exist. Exiting plotting script')
        sys.exit()
    
    varr = np.asarray(nc['SSH'][0,:,:])
...
if ocean == 'hycom':
    fname2d =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.hycom.2d.'+conf['fhhh']+'.nc'
    fname3z =  conf['stormID'].lower()+'.'+conf['ymdh']+'.'+conf['stormModel'].lower()+'.hycom.3z.'+conf['fhhh']+'.nc'

    ncfile2d = os.path.join(conf['COMhafs'], fname2d)
    ncfile3z = os.path.join(conf['COMhafs'], fname3z)
    if os.path.exists(ncfile2d) and os.path.exists(ncfile3z):
        nc2d = xr.open_dataset(ncfile2d)
        nc3z = xr.open_dataset(ncfile3z)
    else:
        print('Both or one of these two files do not exist: ',ncfile2d,' and ',ncfile3z,'. Exiting plotting script')
        sys.exit()

    varr = np.asarray(nc2d['sea_surface_height'][0,:,:])/100 #convert cm to m
    ssu = np.asarray(nc3z['u_velocity'][0,0,:,:])/100
    ssv = np.asarray(nc3z['v_velocity'][0,0,:,:])/100
    lon = np.asarray(nc2d.Longitude)
    lat = np.asarray(nc2d.Latitude)
