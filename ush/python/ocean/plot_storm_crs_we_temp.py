...
"""This scrip plots a longitudinal transect of temperature. """
...
if len(lon_adeck[okfhour])==0 or len(lat_adeck[okfhour])==0:
    ...
    y_eye = lat_adeck[okfhour][0]
    ypos = int(np.round(np.interp(y_eye,lat,np.arange(len(lat)))))
    ...
    ctr = ax.contourf(lon,-zl,var,cmap='Spectral_r',**kw,extend='both')
    cbar = fig.colorbar(ctr,extendrect=True)
    #cbar.set_label('$^oC$',fontsize=14)
    cs = ax.contour(lon,-zl,var,[26],colors='k')
    ax.plot(lon,-mld,'-',color='green')
    #ax.plot(np.tile(lon_eye,len(zl)),-zl,'-k')
    ax.clabel(cs,cs.levels,inline=True,fmt='%1.1f',fontsize=10)
    ax.set_ylim([-300,0])
    ax.set_ylabel('Depth (m)')
    ax.set_xlabel('Longitude')
    ax.set_xticks(lon[::200])
    ax.set_xticklabels(lon_ticks[::200].astype(int))
    ...
    var_info = 'Temperature($^oC$) X-section at  ' + str(np.round(y_eye,2)) + ' ' + hemis
    ...
    pngFile = conf['stormName'].upper()+conf['stormID'].upper()+'.'+conf['ymdh']+'.'+conf['stormModel']+'.ocean.storm.crs_we_temp'+'.'+conf['fhhh'].lower()+'.png'
    ...
    fig,ax = plt.subplots(figsize=(8,4))
    ctr = ax.contourf(lon,-zl,diff,cmap='seismic',**kw,extend='both')
    cbar = fig.colorbar(ctr,extendrect=True)
    #cbar.set_label('$^oC$',fontsize=14)
    cs = ax.contour(lon,-zl,diff,[0],colors='grey',alpha=0.3)
    ax.plot(lon,-mld,'-',color='green')
    #ax.plot(np.tile(lon_eye,len(zl)),-zl,'-k')
    ax.clabel(cs,cs.levels,inline=True,fmt='%1.1f',fontsize=10)
    ax.set_ylim([-300,0])
    ...
    model_info = os.environ.get('TITLEgraph','').strip()
    var_info = 'Temperature Difference($^oC$) X-section at ' + str(np.round(y_eye,2)) + ' ' + hemis + '. Min dT = ' + str(np.round(np.nanmin(diff),2))
    storm_info = conf['stormName']+conf['stormID']
    title_left = """{0}\n{1}\n{2}""".format(model_info,var_info,storm_info)
    ax.set_title(title_left, loc='left', y=0.99,fontsize=8)
    title_right = conf['initTime'].strftime('Init: %Y%m%d%HZ ')+conf['fhhh'].upper()+conf['validTime'].strftime(' Valid: %Y%m%d%HZ')
    ...
    pngFile = conf['stormName'].upper()+conf['stormID'].upper()+'.'+conf['ymdh']+'.'+conf['stormModel']+'.ocean.storm.crs_we_temp'+'.change.'+conf['fhhh'].lower()+'.png'
    plt.savefig(pngFile,bbox_inches='tight',dpi=150)
    plt.close()
