 ...
 *********************************************************************************
 usage: python storm_WvelZ40m.py stormModel stormName stormID YMDH trackon COMhafs
 -----
 *********************************************************************************
...
for k in range(len(aln)):
   ...
   plt.suptitle(storm.upper()+tcid.upper()+'  '+'Ver Hr '+"%3d"%(fhr)+'  (IC='+cycle+'): n 40-m Wvel & Change [m/day]',fontsize=15)
   plt.subplot(121)
   ...
   plt.subplot(122)
   (dvar[k]*dumb).plot.contourf(levels=np.arange(-50,55,5),cmap='bwr')
   plt.plot(cx,cy,color='gray')
   if trackon[0].lower()=='y':
        plt.plot(aln,alt,'-ok',linewidth=3,alpha=0.6,markersize=2)
        plt.plot(aln[k],alt[k],'ok',markerfacecolor='none',markersize=10,alpha=0.6)
   ...
   pngFile=os.path.join(graphdir,aprefix.upper()+'.'+model.upper()+'.storm.Wvel.Z40m.f'+"%03d"%(fhr)+'.png')
   plt.savefig(pngFile,bbox_inches='tight')

   plt.close("all")

#   stats.append([np.nanmean(var[k]*dumb),np.nanmin(var[k]*dumb),np.nanmax(var[k]*dumb),
#     np.nanmean(dvar[k]*dumb),np.nanmin(dvar[k]*dumb),np.nanmax(dvar[k]*dumb)])

#txtFile=os.path.join(graphdir,aprefix+'_r500kmWvelZ40m.txt')
#np.savetxt(txtFile,stats,fmt="%8.4f %8.4f %8.4f %8.4f %8.4f %8.4f")
