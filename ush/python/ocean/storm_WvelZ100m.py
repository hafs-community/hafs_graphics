 ...
 *********************************************************************************
 usage: python storm_tempZ100m.py stormModel stormName stormID YMDH trackon COMhafs
 -----
 *********************************************************************************
...
var=ncfiles['w_velocity'].isel(Z=14)
dvar=var-var[0]
...
for k in range(len(aln)):
   ...
   plt.suptitle(storm.upper()+tcid.upper()+'  '+'Ver Hr '+"%3d"%(fhr)+'  (IC='+cycle+'): 100-m Wvel & Change [m/day]',fontsize=15)
   plt.subplot(121)
   ...
   pngFile=os.path.join(graphdir,aprefix.upper()+'.'+model.upper()+'.storm.Wvel.Z100m.f'+"%03d"%(fhr)+'.png')
